...
class Race(object):
    def __init__(self,year,state,district,status,percentage,party,name):
     ...
     #    	return None
        ...
        self.percentage = getpercent(percentage)
        self.win =  win(self.percentage)
        self.party = changeparty(party)
...
def changeparty(currparty):
	if currparty == "republican":
		return "R"
	elif currparty == "democrat":
		return "D"
	else:
		return "I"
...
def approval(year,state):
	if str.lower(state) == "alabama":
		return 0.41
	with codecs.open("approval.csv", "r" ,encoding='utf-8', errors='ignore') as f:
		for row in csv.reader(f):
			if str.lower(row[0]) == str.lower(state) and float(row[1]) == float(year):
				# print(str.lower(row[0]))
				# print(row[4])
				# print(float(row[4][:-1])/100)
...
if __name__ == "__main__":
	...
	with codecs.open("house2016.csv", "r" ,encoding='utf-8', errors='ignore') as f:
		for row in csv.reader(f):
			#(self,year,state,district,status,percentage,party,name):
			if is_number(row[0]) and row[9] != "" and row[2] != "independent_or_other":
				district = str(row[1]).split()[:1]
				races.append(Race(2016,str.lower(row[8]),district[0][:-2],row[3],row[9],row[2],row[5]))
	# for x in races: 
	# 	print(x)
	...
	with open("2016percent.csv", "w") as f:
		writer = csv.writer(f, delimiter=',')
		for race in races:
			if str(race.presapproal) == "None":
				print(race.state)
				print(race.year)
